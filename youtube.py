...
# API key connection 
def Api_connect():
    Api_Id="AIzaSyA0MwmwLGVgcXLf8RyK2Ov3eGDu7_MQuw0"
    ...
    for i in response["items"]:
        channel_data={'channel_Name': i['snippet']['title'],
                       'channel_Id':i['id'],
                        ...
                        'likes': i['contentDetails']['relatedPlaylists']}
...
def get_video_info(video_ids):
  ...
  for video_id in video_ids:
      request = youtube.videos().list(
              part ="snippet,contentDetails,statistics",
              id=video_id)#---remove the cods('')
      response=request.execute()

      for i in response["items"]:
                  data={'channel_Name':i['snippet']['channelTitle'],
...
uri = "mongodb+srv://saran:saran05@saran5.q8tauew.mongodb.net/?retryWrites=true&w=majority"
...
def channel_details(channel_id):
    ...
    vid_ids=get_channel_videos(channel_id)
    vid_details=get_video_info(vid_ids)
    com_details=get_comment_info(vid_ids)

    coll1=db["channel_details"]
    coll1.insert_one({"channel_information":chl_details,"playlist_information":pl_details,
